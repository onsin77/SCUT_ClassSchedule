...
1. 从 [Releases](../../releases) 或本仓库根目录拿 `华工课程表-v1.1.2.apk`，传到手机（微信 / QQ 传文件、数据线拷贝都行）。
...
| 脚本 | 用途 |
|---|---|
| `node tools/test-logic.js` | **纯逻辑测试，168 项**。给 app.js 一个假 DOM 和一个可控的「现在」，直接测日期计算、配色分配、通知计划、调休、运势概率这些 |
...
> （`node tools/test-logic.js` 一把跑完 168 项）。
